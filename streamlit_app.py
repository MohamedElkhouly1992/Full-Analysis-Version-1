...
def run_catboost_analysis(df, forecast_days):
    ...
    y_energy=features["Total HVAC Energy (kWh)"]
    y_comfort=features["Comfort Deviation"]
    y_deg=features["Degradation Index"]
    y_co2=features["CO2 Production (kg)"]
    horizon=min(forecast_days, len(features))
    if CATBOOST_AVAILABLE and len(features) > 20:
        model_e=CatBoostRegressor(verbose=False, random_seed=42)
        model_c=CatBoostRegressor(verbose=False, random_seed=42)
        model_d=CatBoostRegressor(verbose=False, random_seed=42)
        model_co2=CatBoostRegressor(verbose=False, random_seed=42)
        model_e.fit(X,y_energy); model_c.fit(X,y_comfort); model_d.fit(X,y_deg); model_co2.fit(X,y_co2)
        ...
        out=pd.DataFrame({
            ...
            "Predicted Energy Consumption (kWh)": model_e.predict(future[X.columns]),
            "Predicted Comfort Deviation (C)": model_c.predict(future[X.columns]),
            "Predicted Mean Degradation Index": np.clip(model_d.predict(future[X.columns]),0,1),
            "Predicted Carbon Footprint (kgCO2)": model_co2.predict(future[X.columns]),
            "Model":"CatBoost"
        })
        return out
    future=features.tail(horizon).copy()
    out=pd.DataFrame({
        ...
        "Model":"Fallback rolling mean"
    })
    return out

def build_outputs(all_data, zone_data=None, catboost_df=None, kpi_df=None):
...
st.caption("Enhanced with zones analysis, CatBoost analysis, KPI exports, charts, and framework-style building health outputs.")
...
if weather_file is None:
    ...
    try:
        ...
        for strategy,severity,climate in scenarios:
            ...
            outputs=build_outputs(df, zone_df, cb_df, kpi_df)
        ...
        selected_scenario=st.selectbox("Select scenario for zones/CatBoost preview", list(scenario_data_map.keys()))
        ...
        config_export={"building":asdict(b),"hvac":asdict(h),"degradation":asdict(dcfg),"axis_mode":axis_mode,"fixed_severity":severity_pick,"fixed_strategy":strategy_pick,"fixed_climate":climate_pick,"simulation_years":simulation_years,"forecast_days":int(forecast_days),"zones":int(zone_count)}
