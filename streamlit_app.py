...
def run_catboost_analysis(df, forecast_days):
    ...
    if CATBOOST_AVAILABLE and len(features) > 20:
        ...
        models["energy"].fit(X,features["Total HVAC Energy (kWh)"])
        models["comfort"].fit(X,features["Comfort Deviation"])
        models["deg"].fit(X,features["Degradation Index"])
        models["co2"].fit(X,features["CO2 Production (kg)"])
